...
| `TOSS_CLIENT_KEY` | 🔒 서버 | 지금은 mock 값. 비면 자동 mock 모드 |
| `TOSS_SECRET_KEY` | 🔒 서버 | mock |
| `POINT_CHARGE_CALLBACK_URL` | 🔒 서버 | 실결제 붙일 때 실제 콜백 URL |
| `POINT_WITHDRAW_CALLBACK_URL` | 🔒 서버 | |
...
### 인프라
- [ ] **Supabase 리전 = 서울(ap-northeast-2) 권장.** 다르면 Vercel 서버함수↔DB 왕복 지연↑ (리전 변경은 새 프로젝트 필요 → 초기에 확인)
