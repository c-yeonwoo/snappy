...
**fal.ai 실 보정** — 비용 정책
- **베타: mock 유지(비용 0)**. `FAL_KEY`를 `mock`으로 두거나 미설정 → 클라 톤보정으로 동작
- **정식: 싼 모델 + 가드레일로 전환.** fal은 사용량 과금이고 `clarity-upscaler`는 비싼+느린 축.
  - 권장: **가벼운 얼굴보정 모델**(예: codeformer/gfpgan 류, fal 가격·모델ID 확인)로 `FAL_MODEL` 교체
  - **마진 주의**: 크레딧은 무료(호혜)로도 벌려서, 공짜 크레딧 보정 = 순수 비용. 켤 땐 **일일 한도** 또는 **충전 크레딧 한정** 가드 추가
  - ⚠️ **Vercel 타임아웃**: 업스케일 10s+ → Hobby(10s)면 실패. Pro `maxDuration`↑ 또는 빠른 모델
